...
%Demouchy (2013) exponential
ir=ir+1;
n=2;
Q=450*1000;
A=1e6 % 1/s;
P=15e9; %peierls stress (Pa)
B=2.^((1+(3/n))/2)*A.^(-1/n);
rock(id).rheol(ir).name='GoEXP';
rock(id).rheol(ir).ref='Goetze (1978) exponential';
rock(id).rheol(ir).type=1;
rock(id).rheol(ir).r=@(s,T)A.*exp(-(Q./(RG*T)).*((1-(s./P).^(1/n)).^n));
rock(id).rheol(ir).s=@(r,T)P.*((1-((-(RG.*T./Q).*log(r./A)).^(1./n))).^n);
rock(id).rheol(ir).n=n;rock(id).rheol(ir).Q=Q;
rock(id).rheol(ir).B=P;rock(id).rheol(ir).A=A;rock(id).rheol(ir).m=0;

%Hirth and Kohlstedt, 2003, olivine dis-GBS
ir=ir+1;
n=3.5;
m=2;
Q=400*1000;
A=(6.5e3)*(1e-6)^(n+m+p);
B=2.^((1+(3/n))/2)*A.^(-1/n);
rock(id).rheol(ir).name='HK03DG';
rock(id).rheol(ir).ref='Hirth and Kohlstedt, 2003, olivine dis-GBS';
rock(id).rheol(ir).type=3;
rock(id).rheol(ir).r=@(s,T,g)(s/B).^n.*exp(-Q./(RG*T)).*g.^(-m);
rock(id).rheol(ir).s=@(r,T,g)B.*exp(Q./(n*RG*T)).*r.^(1/n).*g.^(m/n);
rock(id).rheol(ir).n=n;rock(id).rheol(ir).Q=Q;
...
%Hansen et al., 2011, olivine dis-GBS
ir=ir+1;
n=2.9;
m=0.7;
Q=445*1000;
A=(6.31e4)*(1e-6)^(n+m+p);
B=2.^((1+(3/n))/2)*A.^(-1/n);
rock(id).rheol(ir).name='HZK11DG';
rock(id).rheol(ir).ref='Hansen et al., 2011, olivine dis-GBS';
rock(id).rheol(ir).type=3;
rock(id).rheol(ir).r=@(s,T,g)(s/B).^n.*exp(-Q./(RG*T)).*g.^(-m);
rock(id).rheol(ir).s=@(r,T,g)B.*exp(Q./(n*RG*T)).*r.^(1/n).*g.^(m/n);
rock(id).rheol(ir).n=n;rock(id).rheol(ir).Q=Q;
...
rock(id).piezo(ip).geq=@(s)max(...
    (3631e-6).*(ss(iT)/1e6)^(-1.26),...
    (78e-6).*(ss(iT)/1e6)^(-0.61));
rock(id).piezo(ip).ref='Stipp and Tullis 2003 combined regime';
ip=ip+1;
rock(id).piezo(ip).geq=@(s)(3631e-6).*(ss(iT)/1e6)^(-1.26);
rock(id).piezo(ip).ref='Stipp and Tullis 2003 regime 2-3';
ip=ip+1;
rock(id).piezo(ip).geq=@(s)(78e-6).*(ss(iT)/1e6)^(-0.61);
